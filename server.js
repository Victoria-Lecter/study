				...
				parser.parseString(xml, function(err, parsedXml) {
					...
					try {
						...
						Character.findOne({ characterId: characterId }, function(err, character) {
							...
							if(character) {
								return res.status(409).send({ nessage: character.name + ' is already in the database.' });
							}
...
app.get('/api/characters/:id', function(req, res, next) {
	...
	Character.findOne({ characterId: id }, function(err, character) {
		...
		if(!character) {
			return res.status(404).send({ message: 'Character not fond.' });
		}
...
app.get('/api/stats', function(req, res, next) {
	async.parallel([
		...
		function(callback) {
			Character.count({ race: 'Male' }, function(err, maleCount) {
				callback(err, maleCount);
			});
		},
		function(callback) {
			Character.count({ race: 'Female' }, function(err, femaleCount) {
